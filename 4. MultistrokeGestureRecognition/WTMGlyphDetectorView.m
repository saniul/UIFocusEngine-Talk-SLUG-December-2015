...
- (void)loadTemplatesWithNames:(NSString*)firstTemplate, ... 
{
  ...
  for (NSString *glyphName = firstTemplate; glyphName != nil; glyphName = va_arg(args, id))
  {
    if (![glyphName isKindOfClass:[NSString class]])
      ...
      [self loadTemplateWithName:glyphName];
  }
  va_end(args);
}

- (void)loadTemplateWithName:(NSString *)glyphName {
...
- (void)loadTemplatesWithNamesArray:(NSArray*)templateNames {
    for (NSString *glyphName in templateNames) {
        if (![glyphName isKindOfClass:[NSString class]]) {
            continue;
        }
